...
if __name__=='__main__':
    ...
    word_id_lsts, post_lsts, bio_lsts, freq_lsts, e_freq_lsts, pos_lsts = read_data('../train_dev_data/dev.txt')

    with open("../data/dev_sentences.pkl",'wb') as f:
        pickle.dump(post_lsts,f)
    with open("../data/dev_postag.pkl",'wb') as f:
        pickle.dump(pos_lsts,f)

    amap = {'O':0,'B':1,'I':2}

    answers = []
    for anss in bio_lsts:
        answer = []
        for ans in anss:
            a = ans.split('|')
            sub_answer = [amap[aa] for aa in a]
            answer.append(sub_answer)
        answers.append(np.transpose(np.array(answer)))
    with open("../data/dev_answers.pkl",'wb') as f:
